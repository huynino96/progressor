...
def count_library(current_date, library, random=True):
    if random:
        library = randrange(library) if library > 0 else 0
    return {
        'date': current_date,
        'library': library,
    }
...
def generate_dates_library(library):
    today = date.today()
    return [
        count_library(today - timedelta(6), library),
        count_library(today - timedelta(5), library),
        count_library(today - timedelta(4), library),
        count_library(today - timedelta(3), library),
        count_library(today - timedelta(2), library),
        count_library(today - timedelta(1), library),
        count_library(today, library, random=False),
    ]
