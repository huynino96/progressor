...
def count_method(repository_endpoint):
    token = Token()
    ...
    while contents:
        ...
        if file_content.type == 'dir':
            ...
            if file.is_python() or file.is_java() or file.is_php() or file.is_javascript():
                ...
                if inside_content.find('function') > -1 or inside_content.find('def') > -1:
                    if file.is_javascript():  # Javascript
                        javascript += 1
                    elif file.is_python() and inside_content.find('def') > -1:  # Python
                        python += 1
